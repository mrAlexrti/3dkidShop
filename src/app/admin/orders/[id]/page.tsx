  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl">Заказ {order.number}</h1>
        <OrderStatusSelect orderId={order.id} status={order.status} />
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="glass rounded-xl2 p-6 shadow-soft lg:col-span-2">
          <h2 className="mb-4 font-display text-lg">Товары</h2>
          <table className="w-full text-left text-sm">
            <thead className="text-ink/50">
              <tr>
                <th className="pb-2">Товар</th>
                <th className="pb-2">Опции</th>
              ...
              {order.items.map((item) => (
                <tr key={item.id} className="border-t border-pink-100">
                  ...
                  <td className="py-2 text-ink/50">
                    {item.optionsJson ? Object.values(JSON.parse(item.optionsJson)).join(", ") : "—"}
                  </td>
          ...
          <div className="mt-4 space-y-1 border-t border-pink-100 pt-4 text-sm">
            <div className="flex justify-between">
              <span className="text-ink/50">Подытог</span>
        ...
        <div className="glass rounded-xl2 p-6 shadow-soft">
          <h2 className="mb-4 font-display text-lg">Клиент</h2>
          <dl className="space-y-2 text-sm">
            <div><dt className="text-ink/50">Имя</dt><dd>{order.customerName}</dd></div>
            ...
            <div><dt className="text-ink/50">Адрес</dt><dd>{order.shippingAddress}, {order.city}, {order.postalCode}</dd></div>
            <div><dt className="text-ink/50">Оплата</dt><dd>{order.paymentMethod === "card" ? "Картой онлайн" : "При получении"}</dd></div>
            {order.comment && <div><dt className="text-ink/50">Комментарий</dt><dd>{order.comment}</dd></div>}
          </dl>
        </div>
      </div>
    </div>
